...
### Font Family
The system runs **El Messiri** (a custom rounded display face) for headlines and **"El Messiri"** for body, navigation, and UI. Plain Black at weight 500 with negative letter-spacing handles every display headline; El Messiri handles the rest. The fallback stack walks `"El Messiri", Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif` for both.
